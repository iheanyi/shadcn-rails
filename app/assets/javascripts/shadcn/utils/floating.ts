...
const sizeMiddleware = (options: Pick<FloatingOptions, "maxHeight"> = {}) => size({
...
function normalizePlacement(placement: Placement | string): Placement {
  return placements.includes(placement as Placement) ? placement as Placement : "bottom-start"
}
 ...
 */
export function positionFloating(reference: HTMLElement, floating: HTMLElement, options: FloatingOptions = {}): () => void {
  const {
 ...
 * @returns {void}
 */
export function positionAtPoint(floating: HTMLElement, x: number, y: number, options: Pick<FloatingOptions, "maxHeight"> = {}): void {
  const { maxHeight = null } = options

  // Create a virtual reference element at the click point
  const virtualRef: ReferenceElement = {
