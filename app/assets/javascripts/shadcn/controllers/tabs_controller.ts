...
export default class extends Controller<HTMLElement> {
  ...
  connect() {
    ...
    if (initialValue) {
      ...
      const validValues = this.triggerTargets.map((t: HTMLElement) => t.dataset.value)
  ...
  selectTab(event: ShadcnEvent) {
    const trigger = event.currentTarget
    const value = trigger.dataset.value
    this.selectTabByValue(value, true)
  }

  selectTabByValue(value: string, updateUrl = true) {
    // Update triggers
  ...
  handleKeydown(event: ShadcnEvent) {
    const triggers = this.triggerTargets.filter((t: HTMLElement) => !t.disabled)
