...
export default class extends Controller<HTMLElement> {
  ...
  openMenu(index: number) {
    ...
    const trigger = menu.querySelector<HTMLElement>("[data-shadcn--menubar-target='trigger']")
    const content = menu.querySelector<HTMLElement>("[data-shadcn--menubar-target='content']")
  ...
  closeAllMenus() {
    ...
    this.contentTargets.forEach((content: HTMLElement) => {
  ...
  toggleCheckbox(event: ShadcnEvent) {
    const item = event.currentTarget
    if (item.dataset.disabled !== undefined) return
  ...
  selectRadio(event: ShadcnEvent) {
    ...
    if (group) {
      ...
      group.querySelectorAll("[role='menuitemradio']").forEach((radio: HTMLElement) => {
  ...
  closeAllSubs() {
    this.subTriggerTargets.forEach((trigger: HTMLElement) => {
      trigger.setAttribute("aria-expanded", "false")
      trigger.dataset.state = "closed"
    })

    this.subContentTargets.forEach((content: HTMLElement) => {
      content.dataset.state = "closed"
      content.hidden = true
    })
  }
  ...
  clickOutside(event: ShadcnEvent) {
    if (this.isMenuOpen) {
      this.closeAll()
    }
  }
  ...
  get currentMenuItems(): HTMLElement[] {
    ...
    return Array.from(content.querySelectorAll<HTMLElement>("[data-shadcn--menubar-target='item']"))
      .filter(item => item.dataset.disabled === undefined)
  }

  positionContent(trigger: HTMLElement, content: HTMLElement) {
  ...
  positionSubContent(trigger: HTMLElement, content: HTMLElement) {
    content.style.position = "absolute"
    content.style.top = "0"
