...
export default class extends Controller<HTMLElement> {
  ...
  open() {
    ...
    this.focusedIndex = -1
    const currentItem = this.itemTargets.find((item: HTMLElement) => item.dataset.value === this.valueValue)
    if (currentItem) {
  ...
  selectByValue(value: string, dispatch = true) {
    ...
    // Update display
    const selectedItem = this.itemTargets.find((item: HTMLElement) => item.dataset.value === value)
  ...
  get enabledItems() {
    return this.itemTargets.filter((item: HTMLElement) => item.dataset.disabled === undefined)
  }
}
