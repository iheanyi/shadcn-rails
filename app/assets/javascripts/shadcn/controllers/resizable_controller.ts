...
export default class extends Controller<HTMLElement> {
  ...
  connect() {
    ...
    this.handleTargets.forEach((handle: HTMLElement) => {
  ...
  getPanelSize(panel: HTMLElement) {
    const containerSize = this.isHorizontal
      ? this.element.offsetWidth
      : this.element.offsetHeight
  ...
  setPanelSize(panel: HTMLElement, percent: number) {
    panel.style.flexBasis = `${percent}%`
    panel.dataset.panelSize = String(percent)
  }

  collapsePanel(which: string) {
    this.findAdjacentPanels()
    this.storePanelSizes()
  ...
  saveSizes() {
    const sizes = this.panelTargets.map((panel: HTMLElement) => this.getPanelSize(panel))
  ...
  loadSavedSizes() {
    ...
    if (saved) {
      try {
        ...
        this.panelTargets.forEach((panel: HTMLElement, index: number) => {
